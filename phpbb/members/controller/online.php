		...
		foreach ($session_data_rowset as $row)
		{
			...
			*/
			...
			$template_row = [
				...
				'USER_IP'			=> ($this->auth->acl_get('a_')) ? (($mode == 'lookup' && $session_id == $row['session_id']) ? phpbb_get_host_for_ip($row['session_ip']) : $row['session_ip']) : '',
				'USER_BROWSER'		=> ($this->auth->acl_get('a_user')) ? utf8_htmlspecialchars($row['session_browser']) : '',
